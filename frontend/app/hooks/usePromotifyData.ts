...
import type { LegacyFacebookGroup, DayOfWeek } from '../types/promotify';
...
export function usePromotifyData() {
  const [groups, setGroups] = useState<LegacyFacebookGroup[]>(INITIAL_GROUPS);
  ...
  const saveGroups = useCallback((newGroups: LegacyFacebookGroup[]) => {
    setGroups(newGroups);
    try {
  ...
  // Add group
  const addGroup = useCallback((group: Omit<LegacyFacebookGroup, 'id'>) => {
    const newId = `grp-${Date.now()}-${Math.random().toString(36).substring(2, 6)}`;
    const newGroup: LegacyFacebookGroup = { ...group, id: newId };
    saveGroups([newGroup, ...groups]);
  }, [groups, saveGroups]);

  // Update group
  const updateGroup = useCallback((updatedGroup: LegacyFacebookGroup) => {
